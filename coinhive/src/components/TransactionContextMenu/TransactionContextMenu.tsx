...
const TransactionContextMenu = ({ onClose, onDelete, onExport, onImport }: TransactionContextMenuProps) => {
    ...
    return (
        <div className={styles.menu} role="menu" onClick={(event) => event.stopPropagation()}>
            <button type="button" role="menuitem" onClick={() => run(onExport)}><Download size={18} /><span><strong>Export transactions</strong><small>Download matching transaction data</small></span></button>
